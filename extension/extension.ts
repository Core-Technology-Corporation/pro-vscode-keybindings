...
/** Cleans up object/array literals: removes extra spaces, commas, and normalizes formatting. */
function cleanupObjectLiterals(lines: string[]): { lines: string[]; changed: boolean } {
  let changed = false;
  const result = lines.map((line) => {
    const original = line;
